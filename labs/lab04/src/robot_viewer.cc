...
RobotViewer::RobotViewer()
  ...
  //My function for changing the color of one of the robot
  gui->addButton("Change Color of Robot 1", 
					std::bind(&RobotViewer::OnColorChangeBtnPressed, this));
...
// My function for changing a robot color
void RobotViewer::OnColorChangeBtnPressed() {
	color_change = !color_change;
	if (color_change) {
	//change the color of robot 1
	robot_land_->set_robot_color(1, 0);
	} else {
	//change back the color
	robot_land_->set_robot_color(1, 200);
	}
}
...
void RobotViewer::DrawRobot(NVGcontext *ctx, int id, double xpos, double ypos,
                            double xvel, double yvel, double rad, int color) {
  ...
  // change robot color by changing one of the argument in nvgRGBA
  nvgFillColor(ctx, nvgRGBA(color, 200, 200, 255));
  nvgFill(ctx);
...
void RobotViewer::DrawUsingNanoVG(NVGcontext *ctx) {
  ...
  for (int i = 0; i < robot_land_->get_num_robots(); i++) {
	...
	int color = robot_land_->get_robot_color(i);
    DrawRobot(ctx, i, xpos, ypos, xvel, yvel, robot_land_->get_robot_radius(i), color);
  }
