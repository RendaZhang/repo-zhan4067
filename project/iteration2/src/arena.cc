...
void Arena::UpdateRobots(void) {
  ...
  for (auto robot : robots_) {
    ...
    for (auto ent : entities_) {
      ...
      double dist = InRange(robot, ent, DEFAULT_RANGE);
      if (dist != -1) {
    ...
    if (!ec.collided()) {
      for (auto ent : entities_) {
        ...
        CheckForEntityCollision(robot, ent, &ec, robot->collision_delta());
        if (ec.collided()) {
          if (ent->get_entity_type() == kRobot) {
            Robot* R = reinterpret_cast<Robot*>(ent);
            if (R->get_speed() == 0) {
...
double Arena::InRange(const ArenaEntity* const ent1,
...
void Arena::UpdateProximityEvent(ArenaMobileEntity * ent1,
  ...
  double triangle_theta = Positive(std::atan(ent2_r/dist)*180/M_PI);
  double distance_theta = Positive(std::atan2(deltaY, deltaX)*180/M_PI);

  double sensor_lower = Positive(heading_angle - floor(fov / 2) + 180);
  double sensor_upper = Positive(heading_angle + floor(fov / 2) + 180);
  double sensed_lower = Positive(distance_theta - triangle_theta);
  double sensed_upper = Positive(distance_theta + triangle_theta);
