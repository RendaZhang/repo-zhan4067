...
class HomeBase: public ArenaMobileEntity {
  ...
  */
  HomeBase& operator=(const HomeBase& other) {
    ...
    this->id_ = other.id_;
    this->heading_angle_ = other.heading_angle_;
    this->angle_delta_ = other.angle_delta_;
    this->speed_delta_ = other.speed_delta_;
    this->motion_handler_ = other.motion_handler_;
    this->motion_behavior_ = other.motion_behavior_;
    this->sensor_touch_ = other.sensor_touch_;
    return *this;
  }
  ...
  */
  ...
  /**
  * @brief calculate a random number based on the private variable seed_
  */
  int RandNum(void) {
    return rand_r(&seed_) % 10 + 1;
  }
